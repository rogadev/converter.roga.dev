  ...
  describe("convertMp4ToGif", () => {
    it("should convert MP4 to GIF with default options", async () => {
      ...
      expect(mockFFmpeg.exec).toHaveBeenCalledWith([
        ...
        "input.mp4",
        "-vf",
        "fps=12,scale=iw:ih:flags=lanczos",
        "-y",
        "output.gif",
      ]);
    ...
    it("should handle custom fps option", async () => {
      ...
      expect(mockFFmpeg.exec).toHaveBeenCalledWith([
        ...
        "input.mp4",
        "-vf",
        "fps=24,scale=iw:ih:flags=lanczos",
        "-y",
        "output.gif",
      ]);
    });
    ...
    it("should handle high quality conversion with palette generation", async () => {
      ...
      expect(mockFFmpeg.exec).toHaveBeenNthCalledWith(1, [
        ...
        "input.mp4",
        "-vf",
        "fps=20,scale=640:-1:flags=lanczos,palettegen=stats_mode=full",
        "-y",
        "palette.png",
      ]);
      ...
      expect(mockFFmpeg.exec).toHaveBeenNthCalledWith(2, [
        ...
        "palette.png",
        "-lavfi",
        "fps=20,scale=640:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=3",
        "-y",
        "output.gif",
      ]);
    });
    ...
    it("should handle FFmpeg load failure", async () => {
      ...
      // Act & Assert
      await expect(convertMp4ToGif(testFile, {})).rejects.toThrow(
        "Failed to convert MP4 to GIF: Failed to load FFmpeg core",
      );
    });
    ...
    it("should handle file write failure", async () => {
      ...
      // Act & Assert
      await expect(convertMp4ToGif(testFile, {})).rejects.toThrow(
        "Failed to convert MP4 to GIF: Failed to write file",
      );
    });
    ...
    it("should handle FFmpeg execution failure", async () => {
      ...
      // Act & Assert
      await expect(convertMp4ToGif(testFile, {})).rejects.toThrow(
        "Failed to convert MP4 to GIF: FFmpeg execution failed",
      );
    });
    ...
    it("should handle output file read failure", async () => {
      ...
      // Act & Assert
      await expect(convertMp4ToGif(testFile, {})).rejects.toThrow(
        "Failed to convert MP4 to GIF: Failed to read output file",
      );
    });
    ...
    it("should handle fetchFile failure", async () => {
      ...
      // Act & Assert
      await expect(convertMp4ToGif(testFile, {})).rejects.toThrow(
        "Failed to convert MP4 to GIF: Failed to fetch file",
      );
    });
    ...
    it("should handle corrupted video files", async () => {
      ...
      // Act & Assert
      await expect(convertMp4ToGif(corruptedFile, {})).rejects.toThrow(
        "Failed to convert MP4 to GIF: Invalid data found when processing input",
      );
    });
    ...
    it("should handle high fps values", async () => {
      ...
      expect(mockFFmpeg.exec).toHaveBeenCalledWith([
        ...
        "input.mp4",
        "-vf",
        "fps=60,scale=iw:ih:flags=lanczos",
        "-y",
        "output.gif",
      ]);
    });
    ...
    it("should handle very small width values", async () => {
      ...
      expect(mockFFmpeg.exec).toHaveBeenCalledWith([
        ...
        "input.mp4",
        "-vf",
        "fps=12,scale=1:-1:flags=lanczos",
        "-y",
        "output.gif",
      ]);
    });
    ...
    it("should handle very large width values", async () => {
      ...
      expect(mockFFmpeg.exec).toHaveBeenCalledWith([
        ...
        "input.mp4",
        "-vf",
        "fps=12,scale=4096:-1:flags=lanczos",
        "-y",
        "output.gif",
      ]);
    });
    ...
    it("should handle non-Error exceptions", async () => {
      ...
      // Act & Assert
      await expect(convertMp4ToGif(testFile, {})).rejects.toThrow(
        "Failed to convert MP4 to GIF: Unknown error",
      );
    });
  });
});
